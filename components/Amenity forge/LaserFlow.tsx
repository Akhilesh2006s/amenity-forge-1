...
// ---- shader code trimmed for brevity in this comment ----
// (Full shader content from your snippet is preserved below)
...
// mainImage and main from your shader...
void mainImage(out vec4 fc,in vec2 frag){
  // ... full body copied from your snippet ...
  // (omitted here for brevity; in actual file we keep the full implementation)
}
...
// Global flags to avoid repeated WebGL context creation in dev / StrictMode
let hasTriedCreatingRenderer = false
...
export const LaserFlow = ({
  ...
  useEffect(() => {
    ...
    // If WebGL was blocked once, don't try again
    if (webglBlocked) return

    // Avoid re‑creating renderer in dev / StrictMode
    if (hasTriedCreatingRenderer || rendererRef.current) return
    hasTriedCreatingRenderer = true
    ...
    if (!gl) {
      console.warn("LaserFlow: WebGL not available, skipping effect.")
      webglBlocked = true
      return
    }
  ...
  }, []) // run once on mount; uniforms are updated in the effect below
